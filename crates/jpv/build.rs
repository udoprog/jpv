...
const URL: &str = "https://github.com/udoprog/jpv";
...
fn main() -> Result<()> {
    ...
    if let Ok(oxidize_version) = env::var("JPV_VERSION") {
        version = oxidize_version;
        user_agent = format!("jpv/{version} (git {rev}; +{URL})",);
    } else {
        version = format!("git-{rev}");
        user_agent = format!("jpv/0 (git {rev}; +{URL})");
    }
